...
// ╔══════════════════════════════════════════════════════════════╗
// ║ DRDD FACTORY ULTRA AAA PATCH                               ║
// ║ Cinematic material/shader/model enhancement layer          ║
// ╚══════════════════════════════════════════════════════════════╝

(function(){

if (!window.THREE || !window.Factory) return;

const _mkStd = THREE.MeshStandardMaterial;

THREE.MeshStandardMaterial = function(opts={}){

  const o = Object.assign({}, opts);

  if(o.roughness === undefined) o.roughness = 0.58;
  if(o.metalness === undefined) o.metalness = 0.12;

  const m = new _mkStd(o);

  m.envMapIntensity = 1.85;

  // Soft stylized fresnel rim
...
function enhanceModel(root){
  ...
  root.traverse(obj=>{
    ...
    // Rounded scaling pass
    if(obj.geometry){

      obj.geometry.computeVertexNormals();

      // subtle organic proportions
      if(obj.geometry.type.includes('Cylinder')){
        obj.scale.x *= 1.03;
        obj.scale.z *= 1.03;
      }

      if(obj.geometry.type.includes('Sphere')){
        obj.scale.y *= 1.02;
      }
    }
  });

  // Secondary motion for capes/chains
  if(root.userData && root.userData.capeSegs){

    root.userData.capeSegs.forEach((seg,i)=>{

      seg.userData._aaaWave = {
        off: Math.random()*10,
        amp: 0.03 + i*0.02
      };
    });
...
// Wrap every factory creator
Object.keys(window.Factory).forEach(key=>{

  const orig = window.Factory[key];

  if(typeof orig !== 'function') return;

  window.Factory[key] = function(){

    const model = orig.apply(this, arguments);

    try{
      enhanceModel(model);

      // Add subtle idle breathing to all major models
      model.userData.__aaaBreath = {
        t: Math.random()*10
      };

      const oldTick = model.userData.tick;

      model.userData.tick = function(dt){

        const b = model.userData.__aaaBreath;

        if(b){
          b.t += dt || 0.016;

          model.scale.y = 1 + Math.sin(b.t * 2.0) * 0.008;
          model.scale.x = 1 - Math.sin(b.t * 2.0) * 0.003;
          model.scale.z = 1 - Math.sin(b.t * 2.0) * 0.003;
        }
        ...
        if(model.userData.capeSegs){
          ...
          model.userData.capeSegs.forEach((seg,i)=>{
            ...
            const w = seg.userData._aaaWave;

            if(!w) return;

            seg.rotation.x =
              Math.sin(b.t * 2.5 + w.off) * w.amp;
          });
        }
      ...
      console.warn('AAA enhance failed for', key, err);
    }
...
console.log('DRDD FACTORY AAA OVERHAUL ACTIVE');
